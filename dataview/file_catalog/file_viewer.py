...
def _view_las(file_path: str):
    ...
    # NO MULTISELECT. The curves say where they belong -- gamma and SP in
    # track 1, caliper in 2, resistivity in 3 on a log scale, the neutron and
    # density pair in 4 -- so asking the reader to pick four at random before
    # anything is drawn puts the work in the wrong place. The strip lays out
    # the conventional triple-combo and names whatever it did not place.
    _las_log_plot(df, depth_col, curve_names, file_path)
...
def _las_log_plot(df, depth_col, curves, file_path):
    """A true-scale scrolling strip, falling back to the old fixed-height
    plotly tracks only when the strip has nothing it can draw.

    ONE FUNCTION ON PURPOSE. LAS, DLIS, LIS and the PDF curve path all call
    this, so giving the strip its own entry point would leave four call sites
    to drift apart -- which is how this codebase grew two of most things.
    """
    try:
        from dataview.file_catalog import strip_log
        cols = [depth_col] + [c for c in curves if c != depth_col]
        if strip_log.render(df[cols], depth_col=depth_col,
                            file_path=file_path,
                            engine=st.session_state.get("engine")):
            return
    except Exception as _strip_exc:
        # A DISCARDED DIAGNOSTIC IS HOW A BROKEN QUERY COMES TO LOOK LIKE AN
        # EMPTY RESULT. Say what failed, then fall back -- never both fail and
        # go quiet.
        st.caption("Strip log unavailable (%s: %s) - falling back to the "
                   "previous plot." % (type(_strip_exc).__name__, _strip_exc))
    return _las_log_plot_plotly(df, depth_col, curves, file_path)


def _las_log_plot_plotly(df, depth_col, curves, file_path):
    try:
