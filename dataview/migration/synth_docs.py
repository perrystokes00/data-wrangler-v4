...
def _field(w, rng):
    own = str(w.get("field_name", "") or "").strip()
    if own:
        return own
...
def _abbr(w):
    v = str(w.get("province_state", "") or "").strip()
...
def _pdf(path, title, subtitle, ident_rows, blocks, footer,
    ...
    for heading, content in blocks:
        if heading and not scout_shape:
            story.append(Paragraph(heading, hd))
        if isinstance(content, tuple):
            head, rows, widths = content
            _pre = ([[heading] + [""] * (len(head) - 1)]
                    if (scout_shape and heading) else [])
            t = Table(_pre + [head] + rows,
                      repeatRows=2 if _pre else 1,
                      colWidths=[x*inch for x in widths] if widths else None)
            _hr = 1 if _pre else 0        # index of the header row
            _span = [("SPAN", (0, 0), (-1, 0))] if _pre else []
            t.setStyle(TableStyle([
                ("BACKGROUND", (0, _hr), (-1, _hr), colors.HexColor("#e6e6e6")),
                ("FONTNAME", (0, 0), (-1, _hr), "Helvetica-Bold"),
...
def scout_ticket(path, w, rng, shown):
    """A scout ticket in the shape scout_pdf_reader actually reads.

    THE READER WAS BUILT FOR THIS DOCUMENT AND COULD NOT READ IT. Its docstring
    says "the ReportLab export", it recognised the file
    (looks_like_text_ticket -> True), extended_classify_pdf called it a scout
    ticket with confidence 1.0 -- and extract_scout_ticket_text returned {}.
    Every section came back empty, so cat_well_core, cat_well_core_sample and
    cat_well_stimulation stayed at zero however many tickets were generated.

    Three things had to match, and none did:

      * the WELL HEADER table is identified by its FIRST CELL being "API".
        Ours led with Operator, so the header was never found and the whole
        extraction bailed out before reaching any section.
      * every section is found by a BANNER ROW -- section name in cell 0, the
        rest of the row blank -- as the first row of its own table. Ours put
        the heading in a Paragraph ABOVE the table, where pdfplumber does not
        see it as a table row at all.
      * section names and column headers are matched against
        scout_pdf_reader.SECTIONS. "Perforation & Stimulation" is not "Frac
        Stages", and "Initial Production (IP)" is not "Production Summary".

    So this now emits the sections that map, with their exact column headers,
    and the adapter carries them straight into the load_scout contract:

        Stratigraphy -> tops        Frac Stages   -> frac  (stimulation)
        DST          -> dst         Core Sample   -> core  (+ core run)
        Directional Survey -> survey    Production Summary -> ip_rows
    ...
    # ── Well header: API FIRST, and the labels the adapter looks up ──────
    _la, _lo = w.get("surface_latitude"), w.get("surface_longitude")
    loc = ""
    if _la not in (None, "") and _lo not in (None, ""):
        loc = (f"{abs(float(_la)):.6f}{'N' if float(_la) >= 0 else 'S'} "
               f"{abs(float(_lo)):.6f}{'E' if float(_lo) >= 0 else 'W'}")
    ident = [("API", shown),
             ("Well Name", w.get("well_name", "")),
             ("Operator", w.get("operator_name", "")),
             ("Field", w.get("field_name") or _field(w, rng)),
             ("County", w.get("county", "")), ("State", _abbr(w)),
             ("Well Type", w.get("well_type", "OIL")),
             ("Status", w.get("well_status", "PRODUCING")),
             ("Spud Date", w.get("spud_date", "")),
             ("Completion", w.get("completion_date", "")),
             ("Total Depth MD", f"{td:,.0f}"),
             ("Surface Loc", loc)]
    if loc:
        ident += [("Surface Latitude",
                   f"{abs(float(_la)):.6f} {'N' if float(_la) >= 0 else 'S'}"),
                  ("Surface Longitude",
                   f"{abs(float(_lo)):.6f} {'E' if float(_lo) >= 0 else 'W'}")]

    # The rows come from synth_tables so a workbook built for this well
    # and this ticket cannot disagree -- see that module's docstring.
    from dataview.migration.synth_tables import well_tables
    blocks = well_tables(w, rng)

    _pdf(path, "WELL SCOUT TICKET",
         f"{w.get('field_name') or _field(w, rng)} — {w.get('operator_name','')}",
         ident, blocks,
         f"CONFIDENTIAL — {w.get('operator_name','')} — "
         f"{w.get('well_name','')} — Scout Ticket",
         scout_shape=True)
